...
class Blog_Model extends MY_Model {
    ...
    public function get_product_data($pro_id){
        return $this->db->select('*')
            ->from('tbl_product')
            ->join('tbl_category' , 'tbl_category.cat_id = tbl_product.pro_cat_id' , 'left')
            ->where('tbl_product.pro_id' , $pro_id)
            ->get()
            ->row_array();
    }

    public function get_blogs($criteria = array() , $order = array() , $limit = array()){
        $blog_count_tb = "SELECT b_id , b_title , b_pro_id , COUNT(b_pro_id) AS blog_count
            FROM tbl_blog AS blog
            GROUP BY blog.b_pro_id";     
        
    	$query = $this->db->select('pro.pro_id , pro.pro_title , pro.pro_rate , pro.pro_image , blog_count_tb.blog_count , tbl_category.cat_name')
    		->from('tbl_product AS pro')
            ->join('(' . $blog_count_tb . ') AS blog_count_tb' , 'blog_count_tb.b_pro_id = pro.pro_id' , 'left')
            ->join('tbl_category' , 'tbl_category.cat_id = pro.pro_cat_id' , 'left')
    		->where($criteria)
    ...
    public function getMoreData($pro_id , $blog_last_id , $avatar_upload_path , $uploadpath_blog_image){
    ...
    public function loadMoreData($pro_id , $blog_last_id , $avatar_upload_path , $uploadpath_blog_image) {
        $blog_list = $this->getMoreData($pro_id , $blog_last_id , $avatar_upload_path , $uploadpath_blog_image);
        $retHtml = '';
        foreach ($blog_list as $key => $blog) {
            $avatar_image = base_url(DEFAULT_AVATAR_IMGURL);
